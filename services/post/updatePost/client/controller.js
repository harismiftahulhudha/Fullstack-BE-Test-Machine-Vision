...
const debug = require('debug')('app:client|post|updatePost')
...
exports.updatePost = (client) => catchAsync(async (req, res, next) => {
    ...
    let dataPost = await client().getPostById({id: req.params.id})
    if (!dataPost) {
    ...
    const { post, user } = await client().updatePost(request)
