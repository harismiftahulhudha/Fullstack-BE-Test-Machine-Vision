...
module.exports = () => {
    ...
    const client = new AuthService(
        `127.0.0.1:${process.env.SERVER_LOGIN_PORT}`,
        grpc.credentials.createInsecure()
    )

    const getUserByUsernameOrEmail = (body) => {
        return new Promise((resolve, reject) => client.getUserByUsernameOrEmail(body, (err, data) => {
            if (err) {
                return reject(err)
            }
