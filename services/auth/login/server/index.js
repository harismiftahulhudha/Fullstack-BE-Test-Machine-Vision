...
const debug = require('debug')('app:server|auth|login')
...
const addr = `127.0.0.1:${process.env.SERVER_LOGIN_PORT}`
...
const authService = {
    getUserByUsernameOrEmail: async (call, callback) => {
        authInteractor.getUserByUsernameOrEmail(call.request)
            .then(res => {
                if (!res) {
                    callback(null, {})
                } else {
